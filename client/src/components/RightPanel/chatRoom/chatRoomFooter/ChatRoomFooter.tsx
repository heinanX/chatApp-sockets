...
function ChatRoomFooter() {
            ...
            const fetchGif = async () => {
                try {
                    ...
                    if (randomGif) {
                        ...
                        if (smalGif) {
                            if (message.trim() !== '') {
                                sendMessage({ gif: smalGif, message: message.trim(), room: room, username: username, timestamp: timestamp });
                                setMessage('');
                            }
    ...
    return (
        <div className="chatroom-footer">
            ...
            {currentWriters.length > 1 &&
                <div className="currentWriter">
                    <p>{currentWriters[0]?.username} och {currentWriters.length - 1} till skriver... </p>
                    <FontAwesomeIcon
                        className="faFaceLaugh"
                        icon={faFaceLaugh as IconProp}
                        bounce />
